...
namespace ash
{
	...
	void App::loadGameObjects()
	{
		std::unique_ptr<Model> model = std::make_unique<Model>(m_graphics->m_logicalDevice.get(), m_graphics->m_physicalDevice.get(),
			m_graphics->m_swapChain->getImageCount(), m_graphics->m_descriptorSetLayout, m_graphics->m_descriptorPool->getPool(), m_graphics->m_textureSampler);
		...
		//	m_graphics->m_swapChain->getImageCount(), m_graphics->m_descriptorSetLayout, m_graphics->m_descriptorPool->getPool(), m_graphics->m_textureSampler);
		//model2->setOffset({0.0f, 0.0f, 0.0f});
